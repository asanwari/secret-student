...
[![Live Space](https://img.shields.io/badge/Play-Live_Space-fbbf24?logo=huggingface&logoColor=black)](https://huggingface.co/spaces/build-small-hackathon/secret-student)
...
> not the normal warm inference path. If you encounter LLM unavailable errors, please retry.

The current model pair uses about **10 GB of VRAM** when loaded together. It can
...
LLM_MODEL=nvidia/NVIDIA-Nemotron-3-Nano-4B-GGUF:Q4_K_M
LLAMA_CPP_MODEL_REF=nvidia/NVIDIA-Nemotron-3-Nano-4B-GGUF:Q4_K_M
...
```text
LLAMA_CPP_MODEL_PATH=/data/models/nemotron-3-nano-4b/NVIDIA-Nemotron-3-Nano-4B-Q4_K_M.gguf
...
| Demo video | [Watch the Secret Student gameplay demo on YouTube](https://www.youtube.com/watch?v=9ISWiIqox0Y) |
...
flowchart LR
    ...
    API -->|Lessons, chat, typed grading| TEXT[NVIDIA Nemotron 3 Nano 4B GGUF]
...
One approximately 4B NVIDIA Nemotron text model handles lesson generation,
teacher chat, question creation, and typed-answer grading. A separate OpenBMB MiniCPM-V model
...
environment-configurable; Nemotron and MiniCPM-V are defaults, not hard dependencies.

The default pair uses about 10 GB of VRAM when loaded together and is
...
| --- | --- |
| [![NVIDIA](https://img.shields.io/badge/NVIDIA-GPU_Inference-76B900?logo=nvidia&logoColor=white)](https://www.nvidia.com/) | NVIDIA GPU hardware runs CUDA-accelerated llama.cpp for local or Modal-hosted inference, and NVIDIA Nemotron 3 Nano 4B GGUF is the text model used for lessons, chat, question generation, and typed-answer grading. This makes NVIDIA part of both the hardware acceleration story and the core model stack. |
...
- **Model choice and use:** NVIDIA Nemotron 3 Nano 4B handles language-heavy work while
